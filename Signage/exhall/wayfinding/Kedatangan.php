...
$id_lantai = 4; // Kedatangan

$video_aktif = "video/Kedatangan/idle.mp4"; // Default video Kedatangan
...
if (isset($_GET['play'])) {
    ...
    if ($row_vid = mysqli_fetch_assoc($query_vid)) {
        if (!empty($row_vid['video_jalur']) && $row_vid['video_jalur'] !== 'null') {
            $video_aktif = strpos($row_vid['video_jalur'], 'upload/') !== false ? $row_vid['video_jalur'] : "upload/" . $row_vid['video_jalur'];
        }
    }
}
?>
...
<html lang="en">
  ...
  <body>
    ...
    >
      ...
      <video autoplay loop muted playsinline poster="gambar\kedatangan\Kedatangan.png" style="width: 100%; margin-top: -110px; display: block;">
    ...
    <script>
      async function loadDataJalur() {
        try {
          ...
          // AMBIL DATA KEDATANGAN (lantai=4)
          const res = await fetch(`api/get_jalur.php?lantai=4&t=${noCache}`);
          const data = await res.json();
          
          const tenants = data.filter(d => d.type.toLowerCase() === 'tenant');
          const pickups = data.filter(d => d.type.toLowerCase() === 'pickupzone' || d.type.toLowerCase() === 'pickup zone');
          const fasilitas = data.filter(d => d.type.toLowerCase() === 'fasilitas');
              ...
              for(let i=0; i<cols; i++){
                ...
                for(let j=0; j<itemsPerCol; j++){
                  ...
                  if(idx < tenants.length) {
                    ...
                    let linkHref = `Kedatangan.php?play=${t.id_jalur}`; // LINK KEDATANGAN
                    ...
                    if (isIdle) {
                        ...
                        // LOGIKA PATH SAMA PERSIS DENGAN LANTAI 1
                        let logoPath = (t.logo_jalur && t.logo_jalur.trim() !== '' && t.logo_jalur !== 'null') ? (t.logo_jalur.includes('upload/') ? `${t.logo_jalur}` : `upload/${t.logo_jalur}`) : '';
                        tenantHTML += `
                          <a href="${linkHref}" class="ahref">
                            <div class="tenant">
                              ${t.nomor_jalur}.
              ...
              for(let i=0; i<pCols; i++){
                ...
                for(let j=0; j<pItemsPerCol; j++){
                  ...
                  if(idx < pickups.length) {
                    ...
                    let linkHref = `Kedatangan.php?play=${p.id_jalur}`; // LINK KEDATANGAN
                    ...
                    if (isIdle) {
                        ...
                        // LOGIKA PATH SAMA PERSIS DENGAN LANTAI 1
                        let logoPath = (p.logo_jalur && p.logo_jalur.trim() !== '' && p.logo_jalur !== 'null') ? (p.logo_jalur.includes('upload/') ? `${p.logo_jalur}` : `upload/${p.logo_jalur}`) : '';
          ...
          if (fasContainer && fasilitas.length > 0) {
              ...
              fasilitas.forEach((f, idx) => {
                  // LOGIKA PATH SAMA PERSIS DENGAN LANTAI 1
                  let logoPath = (f.logo_jalur && f.logo_jalur !== 'null') ? (f.logo_jalur.includes('upload/') ? `${f.logo_jalur}` : `upload/${f.logo_jalur}`) : '';
                  let linkHref = `Kedatangan.php?play=${f.id_jalur}`; // LINK KEDATANGAN
